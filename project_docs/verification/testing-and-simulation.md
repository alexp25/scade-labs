...
| Syntax check of all Lab 3.1/3.2 Python files | repo root | `python -m py_compile` on every `.py` in `src/lab3_1/solution/` and `src/lab3_2/solution/CruiseControl/` (incl. generated wrappers and the new `evaluate_cc.py`) | **All parse with no syntax errors** — does not prove runtime correctness (needs the sibling `.dll` + a Scade One install) |
...
4. Run `test_counter.py`/`test_limiter.py` (Lab 3.1, expect all PASS) or
   `evaluate_cc.py` (Lab 3.2, expect a `results/summary.csv` PASS/FAIL
   traceability report plus `results/plots/*.png` charts — the standalone
   `tester.py` console demo still exists but is no longer the lesson's
   evaluation path).
...
- Lab 3.2: build shows 0 errors; manual-mode `throttle` equals `accel`
  (`0.5`); the `evaluate_cc.py` scenario checkpoints assert `throttle ==
