...
Python evaluation script, `evaluate_cc_full_report.py`, takes over as the actual
...
- `cc_wrapper.py`, `cc_wrapper.c`, `cc_wrapper.def` — generated
  ...
  `set_point`), `cruise_control_CC_design_Outputs` (`throttle` only), enums
  `SSM_ST_SSM1`/`SSM_ST_SSM2` for the two automaton nesting levels, and a
  ctypes-backed `cruise_control_CC_design` class (`reset()`/`cycle()`). Class
  and attribute names depend on the Scade One version/codegen job target —
  reread this file if regenerating and it differs.
