# test_limiter_advanced.py
# Same tests as test_limiter.py, plus a custom traceability tool: Scade
...
#
# Output (under <project-dir>/results/): limiter_traceability.csv and
# limiter_diagram.png (skipped with a warning if matplotlib isn't
# installed).
#
# Run setup_wrapper.py first to generate limiter_wrapper/limiter_wrapper.py.
#
# By default this looks for blocks.swan / limiter_wrapper/ next to this
...
OPERATOR_NAME = "limiter"
EXPECTED_REQS = ["REQ-LIM-01", "REQ-LIM-02", "REQ-LIM-03"]

test_cases = [
    # (value_in, min_value, max_value, expected_out, req)
...
parser.add_argument("--project-dir", "-d", type=Path, default=Path(__file__).parent,
                     help="Folder containing blocks.swan and limiter_wrapper/"
...
sys.path.insert(0, str(project_dir / "limiter_wrapper"))
...
all_passed = True
csv_rows = []
for i, (value_in, min_v, max_v, expected, req) in enumerate(test_cases, start=1):
    lim.reset()
    ...
    csv_rows.append({
        "tid": f"TC-LIM-{i:02d}", "req": req, "nodes": nodes,
        "inputs": f"value_in={value_in}, min={min_v}, max={max_v}",
...
generated_files = []
try:
    csv_path = write_traceability_csv(csv_rows, results_dir / "limiter_traceability.csv")
...
if swan_path is not None:
    try:
        diagram_path = render_diagram(
            ...
            results_dir / "limiter_diagram.png")
        generated_files.append(diagram_path)
    except ImportError:
        print("  NOTE: matplotlib not installed — skipping limiter_diagram.png"
              " (pip install matplotlib).")
    except Exception as e:  # noqa: BLE001 - diagram is a nice-to-have, must not mask test results
        print(f"  WARNING: could not render limiter_diagram.png: {e}")
