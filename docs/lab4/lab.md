...
**Prerequisites:** [Lab 2 — Applying the SDLC (Python)](../lab2/) (conceptual) &nbsp;·&nbsp; [Lab 3 — Software Requirements Engineering, Scade One &amp; Traceability](../lab3/) (hard prerequisite — Scade One install, Swan basics, and the REQ-01–REQ-08 requirement set all come from there)
...
**Complete [Lab 3](../lab3/) first.** It covers everything you need for this lab: installing Scade One Student Edition, creating a project, declaring typed inputs/outputs, drawing block diagrams, running the simulator, building Python test scripts, and — critically — the REQ-01–REQ-08 requirement set this lab builds and traces. There is no separate Scade One installation step here; if you skipped Lab 3's install instructions, go do that first.
...
> If any of these are unfamiliar, revisit **[Lab 3](../lab3/)** before continuing. The concepts you practised there (typed operators, block diagrams, simulation, test harnesses) are used directly in Parts 3–6 below.
...
> Scade One also supports in-tool **Test Harnesses** (`.swant` files, the same mechanism you used for `counter`/`limiter` in Lab 3) for automated sequence checking. This lab uses a **Python evaluation script** instead (Part 6): test scenarios live in plain CSV files, the script drives the generated model cycle-by-cycle, writes CSV results, and plots the dynamic behaviour over time — giving you the same automation with reporting and traceability closer to what you built in Lab 2.
...
> **Lab 3 connection:** In Lab 3 you built Python test scripts for the `Counter` and `Limiter` operators using this exact API — the same `PythonWrapper`, the same `.cycle()` method, the same attribute-based input/output access. The only difference here is that `cruise_control` is stateful (state machine), so the *sequence* of cycles matters for multi-cycle scenarios — which is exactly why scenarios are stored as short cycle-by-cycle sequences rather than single input/output pairs. Refer to [Lab 3, Part 7](../lab3/#part-7-python-test-script) if the wrapper setup is unfamiliar.
...
Inputs and outputs are grouped under `.inputs.<name>` / `.outputs.<name>` on the generated object, and the cycle method is `.cycle()` — same API as Lab 3's wrapper tests. As in Activity 6B, the job name passed to `PythonWrapper` is a string, not `prj.get_job(...)`.
...
In Lab 2 you maintained a traceability matrix as a Python comment. In Scade One, open the **Requirements** panel and link the following model elements to their REQ IDs — the same mechanism you already practiced in [Lab 3, Activities 2G and 4F](../lab3/#part-9-how-traceability-works-in-scade-one) on the Limiter and Counter:

| Model element | REQ |
|---------------|-----|
| The `cruise_control` node itself | REQ-01, REQ-02, REQ-04 (top-level: the whole state machine implements these) |
...
| The `regulator` node | REQ-07 |
| The `limiter` instances inside `regulator` | REQ-08 |
...
**Check your own work against the reference model.** As explained in [Lab 3, Part 9](../lab3/#part-9-how-traceability-works-in-scade-one), linking a requirement writes a `#pragma requirement <ID> #end` annotation directly into the `.swan` source, right before the linked element. The reference `CC_design.swan` in this course's repository already has one such annotation on the top-level node:
