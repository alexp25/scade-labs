...
| `marked.js` 9.1.6 | Renders `lab.md` to HTML in-browser | `docs/lab2/index.html`, `docs/lab3/index.html`, `docs/lab4/index.html` (cdnjs) | remote (CDN) | runtime, in every visitor's browser | pinned in the `<script src>` URL | If cdnjs is unreachable, lab pages fail to render their content (visible fallback error message in `docs/lab2/index.html`) |
...
| Skulpt (`skulpt.min.js` + `skulpt-stdlib.js`) | In-browser Python execution — Lab 2's live editor and Lab 3's Requirement Traceability Checker | `docs/lab2/index.html`, `docs/lab3/index.html` (skulpt.org) | remote (CDN, not vendored) | runtime, Lab 2 + Lab 3 only | unpinned (whatever `skulpt.org` currently serves) | Both pages' Run/Check buttons are non-functional offline or if skulpt.org is unreachable; each page disables the button and shows an error on load failure. Lab 3's checker is text-parsing only (`blocks.swan` → `#pragma requirement` report) — it does not and cannot run the actual generated model, since that requires a compiled native `.dll` via `ctypes`, which no browser-based Python (Skulpt or otherwise) can execute in a browser sandbox; see `docs/lab3/lab.md` Activity 7F |
...
| `ansys-scadeone-core` | Scade One Python API (`ScadeOne`, `PythonWrapper`) used to generate Python wrappers from Scade One models | `src/lab3/starter/requirements.txt` (`==0.8.2`), `src/lab4/starter/CruiseControl/requirements.txt` (unpinned) | local (pip install) | authoring/regeneration-time, instructor/maintainer only | pinned in Lab 3, **unpinned in Lab 4** (inconsistency, not corrected — would require a maintainer decision on which version to pin) | Also requires a local Scade One Student Edition install (`v261`, per hardcoded install paths) — not just the pip package |
| Ansys Scade One Student Edition (desktop app) | Model editor, simulator, code generator | `docs/lab3/lab.md`, `docs/lab4/lab.md` (install links to ansys.com), `setup_wrapper.py`/`generate_python_wrapper.bat` (hardcoded install path) | local, proprietary desktop install | model-editing, simulation, and code-generation time | no version number stated beyond the `v261` install-path segment | Confirmed this session: does not require registration/license activation for the student edition (corrected from a prior inaccurate lab.md claim) |
...
| Python 3.12 | Runs the Scade One wrapper-generation/test scripts | `setup_wrapper.py`, `generate_python_wrapper.bat`, both labs' `lab.md` (`py -3 ...`) | local | instructor/maintainer, Lab 3/4 only | explicitly `3.12` | Lab 2 has no stated Python version requirement (stdlib-only, ran successfully under 3.11.3 in this environment) |
...
- Lab 2's live-editor path (Skulpt + CodeMirror + marked.js + highlight.js)
  ...
  lab would need to vendor these. Lab 3's Requirement Traceability Checker
  adds the same skulpt.org dependency (no CodeMirror — plain `<textarea>`),
  same caveat.
- `ansys-scadeone-core` version drift between Lab 3 (pinned) and Lab 4
  (unpinned) means a fresh install of Lab 4's wrapper generator could pick
