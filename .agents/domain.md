...
- **Test harness** — a Swan `.swant` file that instantiates an operator
  ...
  harnesses before Part 6's Python evaluation script (`evaluate_cc_full_report.py` +
...
- **Software architecture (component / connector / interface /
  constraint)** — taught in Lab 6 (`docs/lab6/lab.md`), citing ISO/IEC/IEEE
  42010: **component** = a major building block (e.g. Lab 4's
  `cruise_control`, `regulator`, `limiter`, `car` nodes, each treated as one
  component in Lab 6 Part 10); **connector** = how components communicate
  (function calls/instance-block wiring inside `Simulation.swan`, or a
  vehicle-level signal bus in Lab 6 Part 11's whole-system view);
  **interface** = a component's typed signature (a Swan node's
  inputs/outputs, read as provided/required services); **constraint** = a
  non-functional bound the architecture must respect (e.g. `throttle`
  clamped to `[0,100]`, deterministic/no dynamic allocation). Lab 6 also
  introduces a **SysML-style block diagram** (Part 11) — explicitly a
  Mermaid stand-in, not output from a real SysML tool, since this repo has
  none — to describe the whole vehicle, not just the embedded-software
  boundary Lab 4's model lives inside.
